...
module.exports.login = [
  // validation rules
  validator.body('email', 'Please enter Email').isLength({ min: 1 }),
  validator.body('email','Please enter a valid Email').isEmail(),
  validator.body('password', 'Please enter Password').isLength({ min: 1 }),
        ...
        return bcrypt.compare(req.body.password, user.password, function(err, isMatched) {
          if(isMatched===true){
            return res.json({
              ...
              token: jwt.sign({_id: user._id, email: user.email, full_name: user.full_name, admin: user.admin}, config.authSecret) // generate JWT token here
            });
          }
          else{
            return res.status(500).json({
